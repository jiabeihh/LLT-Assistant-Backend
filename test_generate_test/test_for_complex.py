...
POLLING_ENDPOINT_TEMPLATE = "/api/v1/tasks/{task_id}"
...
def get_result_after_wait(task_id, host):
    ...
    while time.time() - start_time < MAX_WAIT_SECONDS:
        ...
        try:
            ...
            if result_info.get("detail") == "Not Found":
                print(f"    -- 任务 {task_id} 尚未完成，继续等待...{result_info}")
                time.sleep(5)
                continue
...
def run_tests(test_dir):
    ...
    report_path = os.path.join(os.path.dirname(test_dir), "test_report.xml")
    command = ["pytest", test_dir, f"--junitxml={report_path}"]
...
def main():
    ...
    code_dir = os.path.join(base_dir, "../data/raw/buggy")
    test_dir = os.path.join(base_dir, "test_code/buggy")
    ...
    for root, _, files in os.walk(code_dir):
        for file in files[0:5]:
            if file.endswith(".py"):
                ...
                if all_tests_for_file:
                    test_file_name = "test_" + file
                    test_file_path = os.path.join(test_dir, test_file_name)

                    imports = set()
                    test_functions = []
                    for code in all_tests_for_file:
                        lines = code.split("\n")
                        for line in lines:
                    ...
                    print(f"  -> 已为 {file} 生成所有测试并合并保存到 {test_file_path}")
